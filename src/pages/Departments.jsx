  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="w-8 h-8 border-4 border-mdt-line-2 border-t-blue-500 rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-mdt-text">Departments</h1>
          <p className="text-mdt-muted text-sm mt-1">{isPlatformAdmin ? "Manage all department rosters" : "Departments you administer"}</p>
        </div>
        {isPlatformAdmin && (
          <Button onClick={() => { setEditing(null); setForm({ name: "", category: "", description: "", max_slots: "", discord_webhook_url: "", shift_quota_weekly: "", ranks: [] }); setShowForm(true); }} className="bg-mdt-accent hover:brightness-110 rounded-none">
      ...
      {visibleDepartments.length === 0 ? (
        <div className="bg-mdt-surface border border-mdt-line p-16 text-center">
          <Shield className="w-12 h-12 text-mdt-dim mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-mdt-text mb-2">{isPlatformAdmin ? "No departments yet" : "No departments assigned"}</h3>
          {isPlatformAdmin ? (
            <>
              <p className="text-mdt-muted mb-4">Create your first department to start building rosters</p>
              <Button onClick={() => setShowForm(true)} className="bg-mdt-accent hover:brightness-110 rounded-none">
                <Plus className="w-4 h-4 mr-2" /> Create Department
              </Button>
            </>
          ) : (
            <p className="text-mdt-muted">You aren't set as an admin for any department yet.</p>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {visibleDepartments.map((dept) => {
            ...
            return (
              <div key={dept.id} className="bg-mdt-surface border border-mdt-line overflow-hidden group">
                <div className="p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-mdt-surface-3 flex items-center justify-center">
                        <Icon className="w-5 h-5 text-blue-400" />
                      </div>
                      <div>
                        <h3 className="font-semibold text-mdt-text">{dept.name}</h3>
                        <p className="text-xs text-mdt-dim">{dept.category}</p>
                      </div>
                    </div>
                    {isPlatformAdmin && (
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(dept)} className="p-1.5 rounded-lg hover:bg-mdt-surface-3 text-mdt-muted">
                          <Edit className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => handleDelete(dept.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-mdt-muted hover:text-red-400">
                  ...
                  {dept.description && (
                    <p className="text-sm text-mdt-muted mt-3 line-clamp-2">{dept.description}</p>
                  )}
                  <div className="mt-4 flex items-center gap-6 text-sm">
                    <div className="flex items-center gap-1.5">
                      <Users className="w-3.5 h-3.5 text-mdt-dim" />
                      <span className="text-mdt-text">{deptMembers.length}{dept.max_slots ? `/${dept.max_slots}` : ""}</span>
                    </div>
                  </div>
                </div>
                <Link
                  to={`/departments/${dept.id}`}
                  className="block px-5 py-3 bg-mdt-surface-2 text-sm text-mdt-accent hover:brightness-125 hover:bg-mdt-surface-3 transition-colors text-center font-medium"
                >
                  View Roster →
                </Link>
              </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="bg-mdt-surface border-mdt-line-2 rounded-none text-mdt-text max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            <div>
              ...
              <Input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1" placeholder="e.g. Los Santos Police" />
            </div>
            <div>
              <Label className="text-mdt-text">Category *</Label>
              <Select value={form.category} onValueChange={v => {
                ...
                <SelectTrigger className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1"><SelectValue placeholder="Select category" /></SelectTrigger>
                <SelectContent className="bg-mdt-bg border-mdt-line-2">
                  {categories.map(c => <SelectItem key={c} value={c} className="text-mdt-text">{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label className="text-mdt-text">Description</Label>
              <Input value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1" placeholder="Brief description" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label className="text-mdt-text">Max Slots</Label>
                <Input type="number" value={form.max_slots} onChange={e => setForm({...form, max_slots: e.target.value})} className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1" placeholder="e.g. 20" />
              </div>
              <div>
                <Label className="text-mdt-text">Weekly Shift Quota</Label>
                <Input type="number" value={form.shift_quota_weekly} onChange={e => setForm({...form, shift_quota_weekly: e.target.value})} className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1" placeholder="e.g. 3" />
              </div>
            </div>
            <div>
              <Label className="text-mdt-text">Discord Webhook URL</Label>
              <Input value={form.discord_webhook_url} onChange={e => setForm({...form, discord_webhook_url: e.target.value})} className="bg-mdt-bg border-mdt-line-2 text-mdt-text mt-1" placeholder="https://discord.com/api/webhooks/..." />
            </div>
            <div>
              <div className="flex items-center justify-between">
                <Label className="text-mdt-text">Ranks</Label>
                <button type="button" onClick={loadDefaultRanks} className="text-xs text-mdt-accent hover:brightness-125">Load Defaults</button>
              </div>
                ...
                {(form.ranks || []).map((rank, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <input
                      ...
                      className="w-8 h-8 rounded border border-mdt-line-2 bg-slate-800 cursor-pointer shrink-0"
                    />
                    <Input
                      value={rank.name}
                      onChange={e => updateRank(idx, "name", e.target.value)}
                      className="bg-mdt-bg border-mdt-line-2 text-mdt-text flex-1 h-8"
                    ...
                    <Input
                      ...
                      className="bg-mdt-bg border-mdt-line-2 text-mdt-text w-20 h-8"
                      placeholder="Level"
                    />
                    <button type="button" onClick={() => removeRank(idx)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-mdt-muted hover:text-red-400 shrink-0">
                      <X className="w-3.5 h-3.5" />
                    </button>
                  </div>
                ))}
                <Button variant="outline" size="sm" type="button" onClick={addRank} className="w-full border-mdt-line-2 text-mdt-text hover:bg-mdt-surface-3">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <Button onClick={handleSave} disabled={!form.name || !form.category} className="bg-mdt-accent hover:brightness-110 rounded-none">
