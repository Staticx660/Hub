...
import { Siren, MapPin, Phone, Plus, Link2, Unlink, Users, AlertTriangle } from "lucide-react";
...
export default function DispatchView({ department, session, setSession }) {
  ...
  const attachToCall = async (callId) => {
    try {
      ...
      const log = [...(call.assignment_log || []), { unit_name: session.callsign || session.user_name, action: "assigned", timestamp: new Date().toISOString() }];
  ...
  const createSelfCall = async () => {
    try {
      const runNum = `RUN-${Date.now().toString().slice(-6)}`;
      const newCall = await base44.entities.ActiveCall.create({
        call_type: "Self-Dispatched", priority: "2 - Medium", status: "Active", location: "Self-Dispatched",
        department_id: department.id, assigned_unit_ids: [session.id], run_number: runNum,
        assignment_log: [{ unit_name: session.callsign || session.user_name, action: "assigned", timestamp: new Date().toISOString() }],
      });
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={createSelfCall} size="sm" variant="outline" className="border-slate-700 text-slate-300 gap-1.5"><Plus className="w-3.5 h-3.5" /> Self-Dispatch</Button>
        </div>
        {myCall && (
        ...
        {calls.length === 0 ? (
          ...
          <div className="space-y-2">
            {calls.map((call) => (
              <div key={call.id} className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 hover:border-slate-700 transition-colors">
                <div className="flex items-start justify-between mb-2">
                  ...
                  {call.id !== session.active_call_id && !myCall && <Button onClick={() => attachToCall(call.id)} size="sm" variant="ghost" className="h-7 text-blue-400 gap-1.5"><Link2 className="w-3 h-3" /> Attach</Button>}
                </div>
