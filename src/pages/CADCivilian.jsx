...
export default function CADCivilian() {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <Button onClick={() => window.history.back()} variant="outline" className="border-slate-700 text-slate-300 gap-2">
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
        <div className="flex items-center gap-3 mb-3">
          <Label className="text-slate-300 text-sm font-semibold whitespace-nowrap">Character</Label>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selectedChar && (
            <Button onClick={() => { setEditingChar(selectedChar); setCharFormOpen(true); }} size="sm" variant="outline" className="border-slate-700 text-slate-300 gap-1.5">
                  ...
                  {warrants.map(w => (
                    <div key={w.id} className="bg-slate-900/60 border border-slate-800 rounded-lg p-3 mb-2">
                      <div className="flex items-center justify-between">
                        <span className="text-white font-medium text-sm">{w.reason}</span>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${w.status === "Active" ? "bg-red-500/15 text-red-400" : "bg-slate-700 text-slate-400"}`}>{w.status}</span>
                      </div>
          ...
          <div className="flex items-start gap-4 mb-4">
            <div className="w-24 h-24 rounded-xl bg-slate-800 border border-slate-700 overflow-hidden flex items-center justify-center flex-shrink-0">
              {selectedChar.photo_url ? (
                ...
                <span className="text-3xl font-bold text-slate-600">{selectedChar.first_name?.[0] || "?"}</span>
              )}
            </div>
            <div>
      ...
      <Dialog open={call911Open} onOpenChange={setCall911Open}>
        <DialogContent className="bg-slate-900 border-slate-700">
          <DialogHeader>
            <DialogTitle className="text-white flex items-center gap-2"><Phone className="w-5 h-5 text-red-400" /> Place 911 Call</DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea value={call911Form.description} onChange={e => setCall911Form({ ...call911Form, description: e.target.value })} className="bg-slate-800 border-slate-700 text-white" rows={3} placeholder="Describe the emergency..." />
            </div>
          </div>
          <DialogFooter>
